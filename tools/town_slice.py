...
def night_image(color, alpha, glass):
    r, g, b = color[..., 0], color[..., 1], color[..., 2]
    ...
    return to_image(np.where(glass[..., None], warm, dark), alpha)
...
def slice_buildings(src, cols, rows, names, units, out, night=False, mask_from=None, masks=None, manifest=True):
    """시트 1장의 칸들을 건물로 저장. names: 칸 순서대로 이름(None/"-" 은 건너뜀).
    masks: 여러 시트에 걸쳐 창문 위치를 빌려 쓰기 위한 공유 dict. 반환: [(meta, report)]"""
    ...
    for idx, name in enumerate(names):
        ...
        if night:
            ...
            night_image(color, alpha, glass).save(out / night_file, optimize=True)
