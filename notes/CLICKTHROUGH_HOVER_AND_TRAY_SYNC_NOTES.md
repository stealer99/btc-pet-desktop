...
## 후속: 활성 상태 시각 표시 (v0.17.30)

- **문제**: 잘 옮겨지지만 "지금 깨어난(조작 가능) 상태"인지 눈으로 알기 어려움.
- **수정**:
  - hover-activate가 `_activate`/`_deactivate`에서 `document.body`의 `ct-awake` 클래스 토글
    (setClickThrough OFF에서도 제거).
  - base.css: `body.ct-awake` → `cursor: grab`(+active grabbing), `#wrap::after`에 **시안-블루
    글로우**(펫 뒤 z-index 5, 은은한 pulse). 무드 글로우(초록/빨강/금)와 색 구분.
- **불변사항**: 큐는 z-index 5(무대 10 뒤)라 **무대 기준선(58px)·HUD(z-index 30) 층위 불변**.
  색은 무드와 겹치지 않게 반드시 시안-블루 계열 유지. `#wrap::after`는 이 용도로만 사용
  (캐릭터 CSS는 .pet/.body 의 pseudo만 씀 — 충돌 없음).
